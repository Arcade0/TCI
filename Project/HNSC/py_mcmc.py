...
def mcmc(
    ...
    if 'sga:' not in sp_sga_l.columns[0]: 
        sp_sga.columns = ['sga:'+ele for ele in sp_sga_l.columns]
    if 'sga:' not in sp_sga_t.columns[0]:
        sp_pro.columns = ['sga:'+ele for ele in sp_sga_t.columns]
    if 'deg:' not in sp_deg_l.columns[0]:
        sp_deg.columns = ['deg:'+ele for ele in sp_deg_l.columns]
    if 'deg:' not in sp_deg_t.columns[0]:
        sp_deg.columns = ['deg:'+ele for ele in sp_deg_t.columns]
